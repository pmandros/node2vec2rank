"""Calibration of N2V2R.significance() under no change, for the old quadratic
degree trend and the natural-spline trend that replaced it.
...
the repository root (about 2 minutes on 4 cores):

    python benchmarks/calibration.py      # -> results/calibration_trend.csv
"""
...
def replicate(scenario, rep):
    ...
    for dims_name, dims in (("default", DEFAULT_DIMS), ("elbow", [elbow])):
        ...
        for trend in TRENDS:
            _, pvalues, qvalues = empirical_null_test(distances, degree, trend=trend)
...
if __name__ == "__main__":
    ...
    results.to_csv(os.path.join(RESULTS_DIR, "calibration_trend.csv"), index=False)
    results["any_call"] = results["num_called_q_0.1"] > 0
    summary = results.groupby(["scenario", "dimensions", "trend"])[
