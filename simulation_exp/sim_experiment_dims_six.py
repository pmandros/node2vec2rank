...
def dbsm_experiment(num_nodes, num_com, num_samples, config_fn, min_comm_prob=0.0, max_comm_prob=0.5, uniform_node_comm_assignment=False, degree_naive=False, degree_ground_truth=False,   noise_gaussian_std=None, multiplicity_model=1, save_dir=None):
    ...
    all_one_n2v2r_non_binary_scores = []
    all_two_n2v2r_non_binary_scores = []
    all_three_n2v2r_non_binary_scores = []
    all_four_n2v2r_non_binary_scores = []
    all_five_n2v2r_non_binary_scores = []
    all_six_n2v2r_non_binary_scores = []
    all_n2v2r_borda_non_binary_scores = []

    all_one_n2v2r_binary_scores = []
    all_two_n2v2r_binary_scores = []
    all_three_n2v2r_binary_scores = []
    all_four_n2v2r_binary_scores = []
    all_five_n2v2r_binary_scores = []
    all_six_n2v2r_binary_scores = []
    all_n2v2r_borda_binary_scores = []

    all_one_n2v2r_recalls = []
    all_two_n2v2r_recalls= []
    all_three_n2v2r_recalls = []
    all_four_n2v2r_recalls = []
    all_five_n2v2r_recalls = []
    all_six_n2v2r_recalls = []
    all_n2v2r_borda_recalls = []




    while len(all_n2v2r_borda_non_binary_scores) < multiplicity_model*num_samples:
        ...
        for j in range(len(list_of_pairs_of_graphs)):
            ...
            n2v2r_ranking_one = rankings.iloc[:, 0]
            n2v2r_ranking_two = rankings.iloc[:, 1]
            n2v2r_ranking_three = rankings.iloc[:, 2]
            n2v2r_ranking_four = rankings.iloc[:, 3]
            n2v2r_ranking_five = rankings.iloc[:,4]
            if len(rankings.columns)==6:
                n2v2r_ranking_six = rankings.iloc[:, 5]
            else:
                n2v2r_ranking_six = rankings.iloc[:, 4]

            borda_ranking = model.aggregate_transform()['1']

            n2v2r_one_ranked_nodes = n2v2r_ranking_one.sort_values(
                ascending=False, inplace=False).index.to_list()
            n2v2r_two_ranked_nodes = n2v2r_ranking_two.sort_values(
                ascending=False, inplace=False).index.to_list()
            n2v2r_three_ranked_nodes = n2v2r_ranking_three.sort_values(
                ascending=False, inplace=False).index.to_list()
            n2v2r_four_ranked_nodes = n2v2r_ranking_four.sort_values(
                ascending=False, inplace=False).index.to_list()
            n2v2r_five_ranked_nodes = n2v2r_ranking_five.sort_values(
                ascending=False, inplace=False).index.to_list()
            n2v2r_six_ranked_nodes = n2v2r_ranking_six.sort_values(
                ascending=False, inplace=False).index.to_list()     
            n2v2r_borda_ranking = borda_ranking.sort_values(
                by='borda_ranks', ascending=False, inplace=False).index.to_list()

            _, n2v2r_one_non_binary_scores, n2v2r_one_binary_scores, n2v2r_one_recalls,_ = evaluate_DBSM_ranking(
                first_block_prob_matrix, second_block_prob_matrix, n2v2r_one_ranked_nodes, assignments, degree_ground_truth=degree_ground_truth)
            
            _, n2v2r_two_non_binary_scores, n2v2r_two_binary_scores, n2v2r_two_recalls,_ = evaluate_DBSM_ranking(
                first_block_prob_matrix, second_block_prob_matrix, n2v2r_two_ranked_nodes, assignments, degree_ground_truth=degree_ground_truth)

            _, n2v2r_three_non_binary_scores, n2v2r_three_binary_scores, n2v2r_three_recalls,_ = evaluate_DBSM_ranking(
                first_block_prob_matrix, second_block_prob_matrix, n2v2r_three_ranked_nodes, assignments, degree_ground_truth=degree_ground_truth)

            _, n2v2r_four_non_binary_scores, n2v2r_four_binary_scores, n2v2r_four_recalls,_ = evaluate_DBSM_ranking(
                            first_block_prob_matrix, second_block_prob_matrix, n2v2r_four_ranked_nodes, assignments, degree_ground_truth=degree_ground_truth)
            
            _, n2v2r_five_non_binary_scores, n2v2r_five_binary_scores, n2v2r_five_recalls,_ = evaluate_DBSM_ranking(
                            first_block_prob_matrix, second_block_prob_matrix, n2v2r_five_ranked_nodes, assignments, degree_ground_truth=degree_ground_truth)
            
            _, n2v2r_six_non_binary_scores, n2v2r_six_binary_scores, n2v2r_six_recalls,_ = evaluate_DBSM_ranking(
                            first_block_prob_matrix, second_block_prob_matrix, n2v2r_six_ranked_nodes, assignments, degree_ground_truth=degree_ground_truth)

            _, n2v2r_borda_non_binary_scores, n2v2r_borda_binary_scores, n2v2r_borda_recalls,most_changing = evaluate_DBSM_ranking(
                first_block_prob_matrix, second_block_prob_matrix, n2v2r_borda_ranking, assignments, degree_ground_truth=degree_ground_truth)
            
            if most_changing!=1:
                continue

            all_one_n2v2r_non_binary_scores.append(n2v2r_one_non_binary_scores)
            all_two_n2v2r_non_binary_scores.append(n2v2r_two_non_binary_scores)
            all_three_n2v2r_non_binary_scores.append(n2v2r_three_non_binary_scores)
            all_four_n2v2r_non_binary_scores.append(n2v2r_four_non_binary_scores)
            all_five_n2v2r_non_binary_scores.append(n2v2r_five_non_binary_scores)
            all_six_n2v2r_non_binary_scores.append(n2v2r_six_non_binary_scores)
            all_n2v2r_borda_non_binary_scores.append(n2v2r_borda_non_binary_scores)


            all_one_n2v2r_binary_scores.append(n2v2r_one_binary_scores)
            all_two_n2v2r_binary_scores.append(n2v2r_two_binary_scores)
            all_three_n2v2r_binary_scores.append(n2v2r_three_binary_scores)
            all_four_n2v2r_binary_scores.append(n2v2r_four_binary_scores)
            all_five_n2v2r_binary_scores.append(n2v2r_five_binary_scores)
            all_six_n2v2r_binary_scores.append(n2v2r_six_binary_scores)
            all_n2v2r_borda_binary_scores.append(n2v2r_borda_binary_scores)

            all_one_n2v2r_recalls.append(n2v2r_one_recalls)
            all_two_n2v2r_recalls.append(n2v2r_two_recalls)
            all_three_n2v2r_recalls.append(n2v2r_three_recalls)
            all_four_n2v2r_recalls.append(n2v2r_four_recalls)
            all_five_n2v2r_recalls.append(n2v2r_five_recalls)
            all_six_n2v2r_recalls.append(n2v2r_six_recalls)
            all_n2v2r_borda_recalls.append(n2v2r_borda_recalls)

    num_samples = len(all_n2v2r_borda_recalls)
    ...
    if save_dir:
        ...
        method = 'n2v2r_one'
        with open(save_dir+"/"+method + "_non_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_one_n2v2r_non_binary_scores)


        method = 'n2v2r_two'
        with open(save_dir+"/"+method + "_non_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_two_n2v2r_non_binary_scores)

        method = 'n2v2r_three'
        with open(save_dir+"/"+method + "_non_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_three_n2v2r_non_binary_scores)

        method = 'n2v2r_four'
        with open(save_dir+"/"+method + "_non_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_four_n2v2r_non_binary_scores)

        method = 'n2v2r_five'
        with open(save_dir+"/"+method + "_non_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_five_n2v2r_non_binary_scores)

        method = 'n2v2r_six'
        with open(save_dir+"/"+method + "_non_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_six_n2v2r_non_binary_scores)

        method = 'n2v2r_borda'
        with open(save_dir+"/"+method + "_non_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_n2v2r_borda_non_binary_scores)



        method = 'n2v2r_one'
        with open(save_dir+"/"+method + "_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_one_n2v2r_binary_scores)


        method = 'n2v2r_two'
        with open(save_dir+"/"+method + "_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_two_n2v2r_binary_scores)

        method = 'n2v2r_three'
        with open(save_dir+"/"+method + "_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_three_n2v2r_binary_scores)

        method = 'n2v2r_four'
        with open(save_dir+"/"+method + "_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_four_n2v2r_binary_scores)

        method = 'n2v2r_five'
        with open(save_dir+"/"+method + "_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_five_n2v2r_binary_scores)

        method = 'n2v2r_six'
        with open(save_dir+"/"+method + "_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_six_n2v2r_binary_scores)

        method = 'n2v2r_borda'
        with open(save_dir+"/"+method + "_binary_scores.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_n2v2r_borda_binary_scores)


        method = 'n2v2r_one'
        with open(save_dir+"/"+method + "_recalls.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_one_n2v2r_recalls)

        method = 'n2v2r_two'
        with open(save_dir+"/"+method + "_recalls.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_two_n2v2r_recalls)

        method = 'n2v2r_three'
        with open(save_dir+"/"+method + "_recalls.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_three_n2v2r_recalls)

        method = 'n2v2r_four'
        with open(save_dir+"/"+method + "_recalls.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_four_n2v2r_recalls)

        method = 'n2v2r_five'
        with open(save_dir+"/"+method + "_recalls.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_five_n2v2r_recalls)

        method = 'n2v2r_six'
        with open(save_dir+"/"+method + "_recalls.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_six_n2v2r_recalls)

        method = 'n2v2r_borda'
        with open(save_dir+"/"+method + "_recalls.csv", "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(all_n2v2r_borda_recalls)
...
def main():
    ...
    save_dir = "../results/final_noise005_nothresh_always_first_dims_six_cose_2-32_new_cor"
