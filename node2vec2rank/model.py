...
class n2v2r():
    def __init__(self, graphs: list, node_names: list, config: dict):
        ...
        self.embed_dimensionÍs = self.config['embed_dimensions']
    ...
    def fit_transform_rank(self):
        ...
        if self.config["verbose"] == 1:
            print(
                f"        Finished UASE embedding in {exec_time_embed} seconds")
        ...
        for i in range(1, self.num_graphs):
            ...
            for d in self.embed_dimensions:
                ...
                for distance_metric in self.distance_metrics:
                    ...
                    rank_pd["dim-"+str(d)+"_distance-" +
                            distance_metric] = distances

            if self.config["save_dir"]:
                rank_pd.to_csv(os.path.join(self.save_dir, str(
