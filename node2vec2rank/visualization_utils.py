...
def plot_3d(component1, component2, component3, color = None, names = None, color_type = 'nominal', save_dir=None, show=True):
    ...
    if names is not None:
        ...
        hovertemplate =
            ...
            text = [f'Name: {names[i]} <br>Cluster: {color[i]}' for i in range(len(names))],
                marker=dict(
                size=4,
    ...
    fig.update_layout(margin=dict( l=100,r=100,b=100,t=100),width=800,height=600)                 
...
def plot_embeddings(embeddings, color = None, names = None, color_type = 'nominal',  save_dir = None, show = True):
    if(embeddings.shape[1]==2):
        plot_2d(embeddings[:,0], embeddings[:,1], color=color, color_type = color_type, names=names,  save_dir=save_dir, show=show)
    else:
...
def dim_reduction(data, algorithm, n_components=3):
    ...
    if algorithm.casefold() == 'tsne'.casefold():
        ...
        raise Exception('Unknown dimensionality reduction algorithm. Options are tsne, umap, isomap, pca')
